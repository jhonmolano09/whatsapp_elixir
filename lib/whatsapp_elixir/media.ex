...
## Args
  ...
  - opts: Map with the following options:
...
def send_location(recipient_id, %{lat: lat, long: long, name: name, address: address} = opts) do
  custom_config = Map.get(opts, :custom_config, [])
...
def send_image(recipient_id, %{image: image} = opts) do
  recipient_type = Map.get(opts, :recipient_type, "individual")
  caption = Map.get(opts, :caption, "")
  link = Map.get(opts, :link, true)
  custom_config = Map.get(opts, :custom_config, [])
